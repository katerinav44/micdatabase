...
def to_bin(filename_txt, filename_png, image):
    ...
    f.close()
    r.save(png_path)
    txt_file_metadata = insert_file(
        service, filename_txt, txt_folder_id, mime_types[0], txt_path
    )
    png_file_metadata = insert_file(
        service, filename_png, image_folder_id, mime_types[1], png_path
    )
    return r
...
def scopus_search(DOI):
    ...
    try:
        ...
        pubdate = datetime.datetime.strptime(pubdate_str, "%d %B %Y").strftime(
            "%d/%m/%Y"
        )
    except KeyError:
        pubdate = ""
...
def add_entry():
    ...
    usecase = st.selectbox(
        "Select a use case",
        options=["OOC", "POC", "chemical analysis", "cell analysis"],
    )
    keywords = st.text_input("Enter any other relevant key words:")
    readout = st.text_input(
        "Briefly describe the specific readout/application of the chip:"
    )
...
def append_new_row(new_row):
    ...
    value_range_body = {"values": [new_row]}
...
def main():
    ...
    if choice == "View data":
        ...
        st.write(
            "Link to google sheets database: https://docs.google.com/spreadsheets/d/1wUghvgCPVcVS6-k-vWXVdbLJUfSg0G8KwQEDCDLsPHc/edit?usp=sharing"
        )
        st.write(
            "Google Drive folder with Images of the Channel Layout: https://drive.google.com/drive/folders/1uTxBRSkS-E6cC71T0EOCprxwB9vtub20?usp=sharing"
        )
        ...
        ax2.pie(
            ...
            shadow=True,
            startangle=90,
            colors=["indigo", "lightblue", "cyan", "mediumblue"],
        )
        ax2.axis("equal")
        st.pyplot(fig2)
        ...
        for key in new:
            ...
            labels3.append(key)
        colors = colors = [
            plt.cm.tab20c(i / float(len(labels3))) for i in range(len(labels3))
        ]
        ...
        st.pyplot(fig4)
        st.write(
            "Please visit the About page to learn more about the database and how to add entries."
        )
        ...
        st.subheader("About")
        st.write(
            "Microfluidics is a field which involves manipulating small amounts of fluids on chips measuring only a few centimeters. In this way, microfluidic chips allow complex chemical/biological processes to be performed with higher throughput, increased automation, and reduced waste. Designing a lab-on-chip device is often a tedious, trial-and-error process for scientists. The behavior of fluids in the microfluidics device depends on numerous parameters (channel dimensions, materials, number of inlets/outlets, etc.) which must be carefully selected by scientists to produce the desired readout. Machine Learning (ML) and applied optimization research have the potential to address the problem of mechanical device design. "
        )
        st.write(
            "The purpose of the microfluidics database is to compile microfluidic device designs documented in the past 20 years of microfluidics research. This database will serve as an important first step towards conducting ML research to optimize device design."
        )
        st.write(
            "Currently, the database is stored in a google sheets file. It contains microfluidic device designs from microfluidics papers written over the past 20 years. There are 18 columns/features :"
        )
        """
